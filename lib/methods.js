Meteor.methods({
    ...
    removeArticle: function(id) {
        ...
        for (i = 0; i < article.attachment_id.length; i++) {
            Attachments.remove(article.attachment_id[i]);
        }
        Articles.remove(id);
    },
    addAttachment: function(att_id, article_id) {
        var attachments = Articles.findOne(article_id).attachment_id;
        if (attachments) {
            attachments.push(att_id);
            Articles.update(article_id, {
                attachment_id: attachments
            });
        }
    },
    removeAttachment: function(att_id, article_id) {
        var attachments = Articles.findOne(article_id).attachment_id;
        if (attachments) {
            //attachments.push(att_id);
            Articles.update(article_id, {
